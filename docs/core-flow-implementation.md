...
```text
/start
  → discover and immediately see an exact Gift
  → name a sender and intended recipient
  ...
  → automatic private handoff name and optional read-only journey
```
...
| --- | --- | --- |
| `GET /` | Explain the product and show curated read-only example paths | Public |
...
| `GET /o/:public_slug` | Show the Gift; authorized holders resume possession without appended administration | Public slug; optional holder cookie |
| `GET /o/:public_slug/journey` | Show the private read-only journey | Encrypted holder or creator cookie |
...
The root landing page uses editable I18n content and local visual assets only. It does not load Gift, Transfer, or JourneyStop records, and its path examples contain no control or capability link. This deliberately avoids turning private journey data into a public feed while still explaining what a path could become.
...
- `Transfers::Claim` locks the Transfer and Gift, makes the first claimant holder generation 1, and creates exactly one JourneyStop with the intended recipient's private handoff name.
...
- A successful claim creates a new holder capability, stores its digest on the Gift, and puts the raw value in an encrypted holder cookie. `/hold/:token` can establish the same role when a raw holder capability is available, but holder recovery is not exposed as an after-opening task.
...
Before claim, `/o/:public_slug` shows only a sealed waiting state. It omits the authored Gift text, context, ritual, intended recipient name, and private note. After claim, it may show the authored Gift and public origin, but not the intended recipient name or private note to an ordinary visitor.

The intended recipient name becomes the private JourneyStop label automatically when the handoff is accepted. It is visible only on the journey route to the creator and authorized current holder. The sender’s private note is visible to the creator and authorized current holder, but not an ordinary public visitor. Creator access does not grant holder controls; holder access does not grant creator controls. Location is not collected or inferred.
...
`GiftVisuals::PrototypeDefault` validates lab selections against `config/gift_visuals.yml`. When a Gift is discovered, `Gifts::Discover` resolves this configuration and stores the small result in `gifts.visual_configuration`. Sender discovery, recipient arrival, and holder view all render from that snapshot. Changing the global default therefore affects only future Gifts; an existing Gift does not change after it has been viewed or sent.
...
- The handoff name is sender-provided and unverified; the current private journey usually contains one stop.
- Passing onward is not implemented. It will later need a new pending Transfer from the current generation, closing the prior JourneyStop only when the next claim succeeds, and holder-token rotation.
